...
`torch.matmul` 或者 `@` 运算符进行矩阵运算的时候，如果参与运算的向量维度大于2，就只会对 -2，-1 两个维度进行矩阵运算，如：
...
Attention 中的 q,k 矩阵，在代码中是一个4维的 `torch.Tensor` 向量，但是实际上只会对最后两个维度计算，符合 Attention 的计算公式
...
机器学习中很喜欢将一堆矩阵全部拼在一起做运算，因为他们在代数上等价，但是运算速度可以快一点



#### 20250702

流形假设：**在高维空间分布的真实数据样本 其实分布在 一个远低于原始维度的低维度流形 (Manifold) 上**

比如一张照片，虽然如果按照向量展开，必将是一个维度极高的向量，但是实际上图像并不是随机的，他们具有很高的规律性，可以很容易的将信息压缩到维度更低的空间中去
...
InceptionV3 是Google的一个预训练的图像分类神经网络 （针对 ImageNet 的1000 分类）

对于两个图象组，我们可以使用 InceptionV3 将其转化为 2048 维向量后计算 FID 的值

在 **FID** 中，我们不关心最终的图像分类结果，而是其中间层（卷积层输出），用它作为图像的语义特征，输出一个 2048 维的向量
...
$\mu_r$：真实图像在特征空间中的均值向量

$\mu_g$：生成图像在特征空间中的均值向量

$\Sigma_r$：真实图像特征的协方差矩阵

$\Sigma_g$：生成图像特征的协方差矩阵

至于为什么是均值，因为 FID 是要计算两组图像之间的距离，实际上是计算两个分布之间的距离
...
注意： FID 是基于分布的指标，不能针对一对单一图像使用，同时对边缘对齐和细节结构不敏感
