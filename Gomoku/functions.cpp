...
constexpr int recursion_depth = 8;
constexpr int next_cell_num = 4;
...
namespace MyState {
	...
	constexpr int Five = 70;
	constexpr int ActiveFour = 50;
	constexpr int RushFour = 18;
	constexpr int ActiveThree = 12;
...
namespace OpState {
	...
	constexpr int Five = -70;
	constexpr int ActiveFour = -55;
	constexpr int RushFour = -30;
	constexpr int ActiveThree = -20;

	const vector<int> score = { Five, ActiveFour, RushFour, ActiveThree };
}

namespace GreedyMyState {

	constexpr int Five = 1000000;
	constexpr int ActiveFour = 300000;
	constexpr int RushFour = 80000;
	constexpr int ActiveThree = 50000;
...
namespace GreedyOpState {
	...
	constexpr int ActiveFour = -600000;
	constexpr int RushFour = -400000;
...
public:
	...
	int CalculateCurrentScore(const vector<vector<CellState>>& cur_board, int turns);
	...
	void GreedyAxisScore(vector<CellState> cur_line, CellState who, vector<int>& Type);

	// Returns a list of potential next moves based on current board state and player
	vector<Position> NextCell(vector<vector<CellState>> chessboard, CellState player, int turns);

	// Chooses the best move based on board evaluation and scoring
	Position BestMovement1();
	Position BestMovement2();

	// Implements Alpha-Beta pruning to evaluate the best move within a given depth and score range
	Movement AlphaBeta(vector<vector<CellState>>& cur_board, int alpha, int beta, CellState player, int depth, int turns);
};
...
vector<Position> ChessBoard::NextCell(vector<vector<CellState>> chessboard, CellState player, int turns) {
	...
	if(turns == 1)
		...
		for (int i = 0; i < static_cast<int>(nextcell.size()); ++i) {
			final_nextcell.push_back(nextcell[i]);
		}
...
Position ChessBoard::BestMovement1() {
	Movement best_move = AlphaBeta(this->board, INT_MIN, INT_MAX, CellState::MY, recursion_depth, 1);
	Position best_position(best_move.x, best_move.y);

	return best_position;
}
Position ChessBoard::BestMovement2() {
	Movement best_move = AlphaBeta(this->board, INT_MIN, INT_MAX, CellState::MY, 1, 2);
	Position best_position(best_move.x, best_move.y);
...
void ChessBoard::GreedyAxisScore(vector<CellState> cur_line, CellState who, vector<int>& Type) {
	...
	for (int type = 0; type < static_cast<int>(Type.size()); ++type) {
		for (auto cur_state : GreedyMyState::mystate[type]) {
			int fptr = 0, cnt = 0;
			bool flag = false;
			for (fptr = 0; fptr < static_cast<int>(cur_line.size()); ++fptr) {
				if (who == CellState::MY ? (cur_line[fptr] == cur_state[cnt]) : (cur_line[fptr] == !cur_state[cnt])) {
...
int ChessBoard::CalculateCurrentScore(const vector<vector<CellState>>& cur_board, int turns) {
	...
	if(turns == 1){
		...
		if ((MyType[2] && MyType[3]) || MyType[3] >= 2)
			return MyState::score[1];
		if ((OpType[2] && OpType[3]) || OpType[3] >= 2)
		...
		vector<CellState> cur_line;
		vector<int> MyType(GreedyMyState::mystate.size(), 0);
		vector<int> OpType(GreedyMyState::mystate.size(), 0);

		// x axis
		for (int i = 0; i < board_length; ++i) {
			cur_line = cur_board[i];
			GreedyAxisScore(cur_line, CellState::MY, MyType);
			GreedyAxisScore(cur_line, CellState::OPPONENT, OpType);
		}
		...
		if (MyType[0])
			return GreedyMyState::score[0];
		if (OpType[0])
			return GreedyOpState::score[0];
		if (OpType[1] || OpType[2])
			return GreedyOpState::score[1];

		int all_score = 0;
		for (int type = 0; type < static_cast<int>(MyType.size()); ++type) {
			all_score += GreedyMyState::score[type] * MyType[type];
		}
		for (int type = 0; type < static_cast<int>(MyType.size()); ++type) {
			all_score += GreedyOpState::score[type] * OpType[type];
		}

		if (MyType[2] && MyType[3])
			all_score += (GreedyMyState::ActiveThree + GreedyMyState::RushFour);
...
Movement ChessBoard::AlphaBeta(vector<vector<CellState>>& cur_board, int alpha, int beta, CellState player, int depth, int turns) {

	// Check for game over or depth limit; return score if reached
	if (GameIsOver(cur_board) || depth <= 0) {
		Movement tmpClass(0, 0, CalculateCurrentScore(cur_board, turns));
		return tmpClass;
	}

	// Get possible next moves based on current board state
	vector<Position> cur_next_cell = NextCell(cur_board, player, turns);
	...
	for (const Position& pos : cur_next_cell) {
		...
		Movement cur_move = AlphaBeta(cur_board, alpha, beta, !player, depth - 1, turns);
	...
	try {
		while (flag) {
			...
			// Turn command computes and outputs AI's best move
				...
				Position my_movement;
				if (move_first == 1)
					my_movement = Board.BestMovement1();
				else
					my_movement = Board.BestMovement2();
