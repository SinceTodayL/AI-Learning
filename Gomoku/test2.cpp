...
void ChessBoard::Init(const CellState& wt) {    // black is the first turn 
...
vector<Position> ChessBoard::NextCell(vector<vector<CellState>> chessboard, CellState player) {
	...
	vector<Position> hasneighbor;
	vector<Movement> nextcell;
	...
	for (Position pos : hasneighbor) {
		...
		vector<int> MyType(MyState::score.size(), 0);
		vector<int> OpType(MyState::score.size(), 0);
